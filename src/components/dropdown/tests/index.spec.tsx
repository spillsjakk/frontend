...
describe("Dropdown Component Unit Tests", () => {
  ...
  beforeEach(() => {
    options = Array(number({ min: 3, max: 10 }))
      .fill(null)
      .map(() => optionFactory());
  });
  ...
  it("should render select and options", () => {
    ...
    const { container } = render(
      <Dropdown options={options} onSelect={(value: string) => null} />
    );
  ...
  it("should call onSelect", async () => {
    ...
    // Act
    const { container } = render(
      <Dropdown options={options} onSelect={onSelectStub} />
    );
    UserEvent.selectOptions(container.querySelector("select"), [
      options[0].value,
    ]);
