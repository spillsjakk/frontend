...
class TeamPlayers extends Component<RouteComponentProps<TeamPlayersProps>, TeamPlayersState> {
  tournamentId: string;
  teamId: string;
  ...
  componentDidMount() {
    ...
    fetchJson(`/s/tournament/manage-team/${this.tournamentId}/${this.teamId}`, "GET", undefined, result => {
  ...
  addParticipant(uid: string) {
    ...
    if (info.tournament.per_team_limit && info.participating.length >= info.tournament.per_team_limit) {
      return;
    }

    fetchJson(`/s/tournament/add-participant/${this.tournamentId}/${uid}?team=${this.teamId}`, "POST", undefined, _ => {
      const participantIndex = info.not_participating.findIndex(p => p[0] === uid);
  ...
  removeParticipant(uid: string) {
    fetchJson(`/s/tournament/remove-participant/${this.tournamentId}/${uid}?team=${this.teamId}`, "POST", undefined, _ => {
      const info = this.state.info!;
      const participantIndex = info.participating.findIndex(p => p[0] === uid);
    ...
    return (
      <>
        ...
        {info.tournament.per_team_limit &&
          <p className="mt-2">
            <strong>{info.tournament.per_team_limit} <Translated str="playersPerTeam" /></strong>
          </p>
        }

        <h3 className="mt-4"><Translated str="participating" /></h3>
        ...
        <table className="table">
          ...
          <tbody id="participating">
            {info.participating.map(player =>
              <tr>
                <td><UserLink id={player[0]} name={player[1] + " " + player[2]} ghost={false} /></td>
                <td><a className="btn btn-danger" onClick={() => this.removeParticipant(player[0])}>X</a></td>
              </tr>
            )}
          </tbody>
        </table>

        <h3 className="mt-4"><Translated str="notParticipating" /></h3>
        ...
        <table className="table">
          ...
          <tbody id="notParticipating">
            {info.not_participating.map(player =>
              <tr>
                <td><UserLink id={player[0]} name={player[1] + " " + player[2]} ghost={false} /></td>
                <td><a className="btn btn-primary" onClick={() => this.addParticipant(player[0])}>+</a></td>
              </tr>
            )}
          </tbody>
        </table>
