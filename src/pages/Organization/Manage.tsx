...
class Manage extends PureComponent<{}, ManageState> {
  ...
  removeClub(cid: string) {
    fetchJson(`/s/organization/remove-club/${this.state.id}/${cid}`, "POST", undefined, _ => {
      const clubs = [...this.state.clubs];
      clubs.splice(clubs.findIndex(c => c.id === cid), 1);
      this.setState({ clubs });
    });
  }
  ...
  addClub(e: FormEvent) {
    ...
    fetchJson(`/s/organization/add-club/${this.state.id}/${this.state.newClubId}`, "POST", undefined, _ => {
    ...
    return (
      <>
        ...
        <form className="mt-4" onSubmit={this.addClub}>
          <div className="form-group">
            <label htmlFor="newClubId"><Translated str="id" />:</label>&nbsp;
            <input type="text" id="newClubId" name="newClubId" required value={this.state.newClubId} onChange={this.handleChange} />&nbsp;
          ...
          <tbody>
            {this.state.clubs.map(club =>
              <tr key={club.id}>
                ...
                <td><a className="btn btn-danger" onClick={() => this.removeClub(club.id)}>X</a></td>
              </tr>
            )}
          </tbody>
        </table>
