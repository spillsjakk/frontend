...
interface Props extends RouteComponentProps<StatisticsProps> {
  popup?: boolean;
}

class Statistics extends Component<Props, StatisticsState> {
  ...
  componentDidMount() {
    ...
    fetchJson(
      ...
      (accounts: Account[]) => {
        fetchJson(
          `/s/organization/clubs/${this.props.match.params.oid}`,
          "GET",
          undefined,
          (clubs: Club[]) => {
            fetchJson(
              `/s/organization/club-members/${this.props.match.params.oid}`,
              "GET",
              undefined,
              (clubMembers: ClubMember[]) => {
                ...
                const regionInfo = lodash.mapValues(
                  lodash.groupBy(clubs, (c) => c.region),
                  (r) => {
                    return {
                      clubCount: r.length,
                      playerCount: 0,
                ...
                const clubDict: { [id: string]: Club } = clubs.reduce(
                  (a, x) => ({ ...a, [x.id]: x }),
                  {}
                );
                const accountDict: { [id: string]: Account } = accounts.reduce(
                  (a, x) => ({ ...a, [x.id]: x }),
                  {}
                );
                ...
                for (const member of clubMembers) {
                  ...
                  if (account.birth_date) {
                    const age =
                      (new Date().getTime() -
                        new Date(account.birth_date!).getTime()) /
                      1000 /
                      60 /
                      60 /
                      24 /
                ...
                this.setState({
                  clubCount,
                  playerCount,
                  regionInfo: Object.keys(regionInfo)
                    .map((k) => {
                      return { name: k, ...regionInfo[k] };
                    })
          ...
          <tbody>
            <tr>
              <td style={{ paddingRight: "65px" }}>
                <Translated str="totalClubCount" />
              </td>
              <td>{this.state.clubCount}</td>
            </tr>
            <tr>
              <td>
                <Translated str="totalPlayerCount" />
              </td>
        ...
        <table className="table mt-5">
          <thead>
            <tr>
              ...
              <th>
                <Translated str="playerCount" />
              </th>
              <th>
                <Translated str="averageAge" />
              </th>
              <th>
          ...
          <tbody>
            {this.state.regionInfo.map((region) => (
              <tr key={region.name}>
                ...
                <td>
                  {Math.round(
                    (region.ageInfo.total / region.ageInfo.count) * 10
                  ) / 10 || ""}
                </td>
                <td>
                  {Math.round(
                    (region.mfRatioInfo.males /
                      (region.mfRatioInfo.total - region.mfRatioInfo.males)) *
                      1000
                  ) / 1000 || ""}
                </td>
                <td></td>
          ...
          <div className="m-5">
            ...
            <PieChart width={350} height={350}>
              <Pie
                ...
                outerRadius={150}
              >
                {this.state.regionInfo.map((_, index) => (
